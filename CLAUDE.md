...
- **День теста максимума — не тренировочный.** После входного теста и после ретеста первая
  тренировка не раньше чем через 48 ч: тест — это тоже подход до отказа.
...
Пороги Lighthouse откалиброваны по прогонам на раннерах CI: FCP ~1.81 с, LCP ~1.98–2.12 с,
performance 0.92–0.98. Всё это `error`. Единственный `warn` — TBT: на холодном прогоне он
скачет до 299 мс против 6–9 мс на прогретых, поэтому лимит 400 мс и мягкий режим.
